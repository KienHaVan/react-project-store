...
const CartPage = () => {
  ...
  useEffect(() => {
    ...
    cartItemCopy.map((item) => {
      ...
      console.log("file: CartPage.js ~ line 14 ~ cartItemCopy", cartItemCopy);
      cartItemHandled.push({
        item,
        count,
      });
    });
    cartItemHandled.filter((item) => item.count !== 0);
    console.log(
      "file: CartPage.js ~ line 21 ~ cartItemHandled",
      cartItemHandled
    );

    // setCartItemHandled(cartItemHandled);
  }, [cartItem]);
