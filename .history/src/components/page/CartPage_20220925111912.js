...
function CartList({ data }) {
  const { itemCount, setItemCount } = useNavbar();
  setItemCount(data.reduce((total, item) => total + item.count));
...
function CartItem({ item }) {
  ...
  const handleDecreaseQuantity = () => {
    if (quantity === 1) {
      ...
      setCartList((prev) => {
        const item = prev.find((one) => one.id === cartItem.id);
      });
    }
  };
    ...
    isValid && (
      <div className=" basis-2/3 flex w-full items-start gap-5 bg-blue-100 rounded-xl p-4">
        ...
        <div className="flex flex-col items-end gap-5">
          ...
          <button className="text-secondary" onClick={() => setIsValid(false)}>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
              fill="currentColor"
