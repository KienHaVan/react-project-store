...
const CartPage = () => {
  ...
  function countCartItem(cartItem, miniItem) {
    ...
    setCartCount(cartItemCopy.filter((item) => item.id === miniItem.id).length);
    const cartItemCounted = cartItemCopy.filter(
      (item) => item.id !== miniItem.id
    );
    return { cartItemCounted, cartCount };
  }
  return (
      ...
      <div className="cartList-right basis-1/3 flex-shrink-0">
        ...
        <div className="flex flex-col gap-5 bg-blue-100 rounded-xl p-3 mb-3">
          <div className="flex items-center gap-3">
            ...
            <span className="font-bold text-xl">Tóm tắt đơn hàng</span>
          </div>
          <div className="flex items-center justify-between">
            <span>Tạm tính</span>
            <span>61.970.000</span>
          </div>
          ...
          <div className="flex items-center justify-between pb-5 border-b border-b-gray-400">
            <span>Tổng cộng</span>
            <span className="text-primary">61.970.000</span>
          </div>
          <button className="p-3 rounded-xl bg-primary text-white font-semibold text-lg">
            Đặt hàng {">"}
          </button>
        </div>
        <div className="flex items-start p-3 rounded-xl border border-green-500 bg-green-100 gap-3">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
