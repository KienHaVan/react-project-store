...
const ProductList = () => {
  ...
  const [url, setUrl] = useState(
    `https://api.escuelajs.co/api/v1/categories/${item.id}/products?offset=${itemOffset}&limit=20`
  );
  ...
  useEffect(() => {
    if (searchDebounce) {
      ...
      setUrl(
        `https://api.escuelajs.co/api/v1/categories/${item.id}/products?offset=0&limit=20`
      );
    }
