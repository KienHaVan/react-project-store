...
function FooterItem({ icon, label, content }) {
  return (
    <div className="w-1/2 flex flex-col gap-3">
      ...
      <span>{content}</span>
      <button className="flex items-center justify-start gap-1 text-secondary hover:text-blue-600">
...
function FooterSocial({ logo, label }) {
  return (
    <div className="p-2 border border-gray-200 rounded-xl flex items-center justify-start gap-3 min-w-[160px] cursor-pointer">
      <span>{logo}</span>
      <span>{label}</span>
    </div>
  );
